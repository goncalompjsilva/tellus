...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update({
        ...
        (this.uniforms!.dt as { value: number }).value = dt;
        this.line.visible = isBounce;
        (this.uniforms!.isBFECC as { value: boolean }).value = BFECC;
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: SimProps & { dst_: THREE.WebGLRenderTarget }) {
        super({
          material: {
            vertexShader: face_vert,
      ...
      update({
        ...
        (this.uniforms!.v as { value: number }).value = viscous;
        for (let i = 0; i < iterations; i++) {
          ...
          (this.uniforms!.velocity_new as { value: THREE.Texture }).value =
            fbo_in.texture;
    ...
    class Divergence extends ShaderPass {
      ...
      update({ vel }: { vel: THREE.WebGLRenderTarget }) {
        (this.uniforms!.velocity as { value: THREE.Texture }).value =
          vel.texture;
        super.update();
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update({ iterations }: { iterations: number }) {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          (this.uniforms!.pressure as { value: THREE.Texture }).value =
            p_in.texture;
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        for (const key in this.fbos) {
          (this.fbos as Record<string, THREE.WebGLRenderTarget>)[key] =
        ...
        for (const key of keys) {
          if (!(this.fbos as Record<string, THREE.WebGLRenderTarget>)[key]) {
            (this.fbos as Record<string, THREE.WebGLRenderTarget>)[key] =
              new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y, opts);
          }
        }
      ...
      resize() {
        this.calcSize();
        for (const key in this.fbos) {
          (this.fbos as Record<string, THREE.WebGLRenderTarget>)[key].setSize(
            this.fboSize.x,
            this.fboSize.y,
          );
        }
    ...
    function createWebGLManager() {
      return class WebGLManager {
        props: {
          $wrapper: HTMLElement;
          autoDemo: boolean;
          autoSpeed: number;
    ...
    const WebGLManager = createWebGLManager();
