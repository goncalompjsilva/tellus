...
export const CollaborateSection = () => {
  ...
  const tags = [
    ...
    { pt: 'Organizações da sociedade civil', en: 'Civil society organizations' },
  ...
  return (
    <section id="colaborar" className="relative noise-texture bg-background py-20 lg:py-[100px]">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          <div className="lg:w-[60%] scroll-reveal">
            ...
            <h2 className="font-display text-3xl lg:text-[48px] font-medium mt-3 leading-tight">
              {t('Trabalhar em rede para amplificar o impacte', 'Networking to amplify impact')}
            </h2>
            ...
            <div className="flex flex-wrap gap-2 mt-6">
              {tags.map((tag) => (
                <span key={tag.pt} className="bg-tellus-off-white border border-tellus-divider font-body text-[13px] text-tellus-body px-3.5 py-1.5 rounded">
                  {t(tag.pt, tag.en)}
                </span>
              ))}
            </div>
          ...
          {/* Right — 40% form */}
          <div className="lg:w-[40%] scroll-reveal" style={{ transitionDelay: '150ms' }}>
            <div className="border border-tellus-divider rounded-lg p-8 lg:p-12 letter-paper">
              {submitted ? (
                <div className="text-center py-8">
                  <p className="font-display text-2xl text-tellus-green">{t('Obrigado!', 'Thank you!')}</p>
                  <p className="font-body text-sm text-tellus-muted mt-2">{t('Entraremos em contacto brevemente.', 'We\'ll be in touch shortly.')}</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label className="font-body text-xs text-tellus-muted block mb-1">{t('Nome', 'Name')}</label>
                    <input type="text" required className="input-underline" />
                  </div>
                  <div>
                    <label className="font-body text-xs text-tellus-muted block mb-1">{t('Organização', 'Organization')}</label>
                    <input type="text" className="input-underline" />
                  </div>
                  <div>
                    <label className="font-body text-xs text-tellus-muted block mb-1">Email</label>
                    <input type="email" required className="input-underline" />
                  </div>
                  <div>
                    <label className="font-body text-xs text-tellus-muted block mb-1">{t('Mensagem', 'Message')}</label>
                    <textarea required rows={3} className="input-underline resize-none" />
                  </div>
                  <button
                    type="submit"
